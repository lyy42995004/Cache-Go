...
func TestCacheBasic(t *testing.T) {
	...
	t.Run("LRU顺序维护", func(t *testing.T) {
		...
		if len(evictedKeys) != 0 {
			t.Fatalf("不应该淘汰，却淘汰%s", evictedKeys[0])
		}
		...
		if evictedKeys[0] != "key3" {
			t.Fatalf("应淘汰key3，实际淘汰%s", evictedKeys[0])
		}
...
func TestCacheAdjust(t *testing.T) {
	...
	// 将key1移动到链表头部
	c.adjust(idx1, pred, suc)

	// 验证key1现在是最近使用的
	if c.dlnk[0][suc] != idx1 {
		t.Errorf("Expected key1 to be at the head of the list")
	}

	// 将key1移动到链表尾部
	c.adjust(idx1, suc, pred)

	// 验证key1现在是最少使用的
	if c.dlnk[0][pred] != idx1 {
