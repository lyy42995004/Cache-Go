...
import (
	...
	gcache "github.com/lyy42995004/Cache-Go"
)

func main() {
	// 命令行参数，用于区分不同节点
	port := flag.Int("port", 8081, "节点端口")
	...
	// 创建新节点
	node, err := gcache.NewServer(addr, "g-cache", 
		gcache.WithEtcdEndpoints([]string{"localhost:2379"}),
		gcache.WithDialTimeout(5*time.Second),
	)
	if err != nil {
	...
	// 创建节点选择器
	picker, err := gcache.NewClientPicker(addr)
	if err != nil {
		log.Fatal("创建节点选择器失败:", err)
	}

	// 创建缓存组
	group := gcache.NewGroup("test", 2<<20, gcache.GetterFunc(
		func(ctx context.Context, key string) ([]byte, error) {
			log.Printf("[节点%s] 触发数据源加载: key=%s", *nodeID, key)
			return fmt.Appendf(nil, "节点%s的数据源值", *nodeID), nil
		}),
	)
	...
	localValue := fmt.Appendf(nil, "这是节点%s的数据", *nodeID)
